...
For AR2, we are splitting each studio in half so that you'll have twice the time (about 20 minutes) to conduct the review session.
The other major difference this time is that you can assume the other teams are already familiar with your project idea, so you can spend less time on introduction and more time going in depth on issues specific to your project.
